...
function createCSS(styles, sheet, lastModified) {
    ...
    if (lastModified && cache) {
        ...
        try {
            ...
            if(checkLocalStorageSize(cache) > 4.99) {
                        log('Localstorage size is over 5mb.');
            }
            log('failed to save');
        }
    }
}
function checkLocalStorageSize(cache) {
    var thatCache = cache,
        cacheSize = 0;
    for(var x in thatCache) {
        cacheSize += parseInt(((thatCache[x].length * 2)/1024/1024).toFixed(2));
    }
    return cacheSize;
},
